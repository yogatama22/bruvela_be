  ...
  <div class="space-y-6">
    <div class="flex items-center space-x-4">
      ...
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Buat pesanan baru dari pelanggan</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div class="space-y-6">
        ...
        <div class="flex flex-col gap-3">
          ...
          <UButton to="/orders" color="gray" variant="outline" size="lg" block>
            Batal
          </UButton>
        </div>
      </div>
...
const loadProducts = async () => {
  ...
  products.value = data || []
  productMap.value = (data || []).reduce((acc: any, p: any) => {
    acc[p.id] = p
    return acc
  }, {})
}
...
const saveOrder = async () => {
  ...
  if (!activeBatch.value) {
    toast.add({
      title: 'Error',
      description: 'Tidak ada batch aktif. Silakan aktifkan batch terlebih dahulu.',
      color: 'red'
    })
    return
  }
  ...
  if (error) {
    toast.add({
      title: 'Error',
      description: error.message || 'Gagal menyimpan order',
      color: 'red'
    })
    return
  }
